...
# ATM: 4/24/2017 - Experiment inital design based off of the following paper:
#
#
#
# This experiment is a collaborative study with Dr Matthew Sutherland
# the purpose of this experiment is to evaluate whether participants who 
# abstain from smoking are impaired in learning from punishmen compared to reward.
# 
# Parameters of the task:
# Within a single run four stimuli are learned simlutaneously
# Probabilistic feedback is provided

trls_per_cond = 40
prob_valid_fb = 0.8
prob_invalid_fb = 0.2

# In addition to the 160 learning trials (40 * 4) participants will see 40 perceptual baseline trials

num_bl_trls = 40

# Total number of trials within a single run is 200
# Each trail lasts 3000 ms (500 ms ISI and 2500 ms stimulus and feedback presentation)
#
# Total length of a single run should be ~10 minutes (**IF** we use TR = 0.994 we would need 603 acquisitions)


# Store info about the experiment session
expName = 'quarters_nic.py'
expInfo = {'participant':'', 'run': 1, 'scanner': 0}
...
filename = _thisDir + os.sep + 'quarters_nic_data/%s_%s_%s' %(expInfo['participant'], expName, expInfo['date'])
...
#Our own log file
datFile=open('quarters_nic_data' + os.sep + '%s_quartersNIC_%s.txt' %(expInfo['participant'], expInfo['date']),'a')
if expInfo['run'] == 1:
...
def BaselineTrial(bl_dict):
    ...
    bltarget_image = (np.random.rand(1280, 800) > target_change) * 1
    blfoil_image = (np.random.rand(1280, 800) > foil_change) * 1
...
#Initialise components for Routine "leadin_fix"
leadin_fix_textClock=core.Clock()
leadin_fix_text=visual.TextStim(win=win, ori=0, name='leadin_fix_text',
    text='+',
    font='Arial',
    pos=[0, 0], height=0.1,wrapWidth=None,
    color=u'black', colorSpace='rgb', opacity=1,
...
#Initialise components for Routine "leadout_fix"
leadout_fix_textClock=core.Clock()
leadout_fix_text=visual.TextStim(win=win, ori=0, name='leadout_fix_text',
    text='+',
    font='Arial',
    pos=[0, 0], height=0.1,wrapWidth=None,
    color=u'black', colorSpace='rgb', opacity=1,
...
#------Prepare to start Routine "leadin_fix"-------
t = 0
leadin_fix_textClock.reset()  # clock 
frameN = -1
# update component parameters for each repeat
# keep track of which components have finished
leadin_fix_textComponents = []
leadin_fix_textComponents.append(leadin_fix_text)
for thisComponent in leadin_fix_textComponents:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED

#-------Start Routine "leadin_fix"-------
continueRoutine = True
while continueRoutine:
    # get current time
    t = leadin_fix_textClock.getTime()
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    routineTimer.add(6.000000)
    # update/draw components on each frame
    
    # *Waiting* updates
    if t >= 0.0 and leadin_fix_text.status == NOT_STARTED:
        # keep track of start time/frame for later
        leadin_fix_text.tStart = t  # underestimates by a little under one frame
        leadin_fix_text.frameNStart = frameN  # exact frame index
        leadin_fix_text.setAutoDraw(True)
    elif leadin_fix_text.status == STARTED and t >= (0.0 + (6.0-win.monitorFramePeriod*0.75)): #most of one frame period left
            leadin_fix_text.setAutoDraw(False)
...
#------Prepare to start Routine "leadout_fix"-------
t = 0
leadout_fix_textClock.reset()  # clock 
frameN = -1
# update component parameters for each repeat
# keep track of which components have finished
leadout_fix_textComponents = []
leadout_fix_textComponents.append(leadout_fix_text)
for thisComponent in leadout_fix_textComponents:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED

#-------Start Routine "leadout"-------
continueRoutine = True
while continueRoutine:
    # get current time
    t = leadout_fix_textClock.getTime()
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    routineTimer.add(6.000000)
    # update/draw components on each frame
    
    # *Waiting* updates
    if t >= 0.0 and leadout_fix_text.status == NOT_STARTED:
        # keep track of start time/frame for later
        leadout_fix_text.tStart = t  # underestimates by a little under one frame
        leadout_fix_text.frameNStart = frameN  # exact frame index
        leadout_fix_text.setAutoDraw(True)
    elif leadout_fix_text.status == STARTED and t >= (0.0 + (6.0-win.monitorFramePeriod*0.75)): #most of one frame period left
            leadout_fix_text.setAutoDraw(False)
